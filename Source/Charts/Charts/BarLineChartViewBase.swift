...
open class BarLineChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
    ...
    internal func drawGridBackground(context: CGContext)
    {
        ...
        {
            context.drawLinearGradient(gradient, start: CGPoint(x: viewPortHandler.contentRect.width, y: viewPortHandler.offsetTop), end: CGPoint(x: viewPortHandler.contentRect.width, y: viewPortHandler.contentRect.height), options: CGGradientDrawingOptions(rawValue: UInt32(0)))
        }
